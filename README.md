...
A tower defense game built as an object-oriented design exercise. Enemies march along a path on a grid map, players place and upgrade towers to stop them before lives run out.
The project demonstrates five classic OO design patterns woven throughout the architecture.
...
`HudView` implements `ITowerDefenseObserver` and registers itself with the bus on creation; it detaches when a new game starts to avoid stale listeners. `GameState` never imports any UI class — it only calls `EventBus.getInstance().postEvent(...)`.

---

### Builder — `GameMap.Builder`

**`GameMap`** (`model/GameMap.java`) has a private constructor that only `Builder` can call, enforcing the pattern. The builder accumulates configuration through a fluent API and materialises the map in one `build()` call.
